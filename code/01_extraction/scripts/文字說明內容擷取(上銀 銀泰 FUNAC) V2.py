...
def is_page_useful(text: str) -> bool:
    lines = [line.strip() for line in text.split('\n') if line.strip()]
    if not lines:
        return False
    ...
    for line in lines:
        ...
        if digits / len(no_space_line) > 0.6 or re.search(r'\d+-\d+', line):
            data_line_count += 1

    data_density = data_line_count / len(lines)
    punctuation_count = sum(1 for line in lines if re.search(r'[。．，,，:；;！?!\.]', line))
    has_description = punctuation_count >= 2 or bool(re.search(r'說明|特性|性能|用途|優勢|适用|注意|描述|Description|Specification|Feature|Note|Formula|Calculation', text, re.I))

    if data_density > 0.4 and not has_description:
        return False
    if len(text) > 1200 and punctuation_count < 3 and data_density > 0.25:
        return False

    alpha_words = re.findall(r'\b[a-zA-Z]{2,}\b', text)
    alpha_word_char_count = sum(len(w) for w in alpha_words)
    data_chars = len(re.findall(r'[\d\.\-/％%×±]', text))
    if alpha_word_char_count + data_chars > 0:
        semantic_ratio = alpha_word_char_count / (alpha_word_char_count + data_chars)
        if semantic_ratio < 0.25 and data_density > 0.3:
            return False
...
def is_table_heavy_page(text: str, page) -> bool:
    if not is_page_useful(text):
        return True

    stats = page_text_stats(text, page)
    if stats['data_line_ratio'] > 0.75 and stats['punctuation_line_ratio'] < 0.05:
        return True
    if stats['line_count'] > 300 and stats['digit_ratio'] > 0.04:
        return True
    if stats['table_marker_ratio'] > 0.5:
        return True
    return False
...
def is_description_block(block: str) -> bool:
    ...
    text_ratio = (len(text) - digits) / len(text) if len(text) else 0.0
    spaces_pattern = len(re.findall(r'\s{2,}', text))

    if spaces_pattern >= 3 and punctuation_count == 0:
        return False
    if text_ratio < 0.2:
        return False
    if punctuation_count > 0 and text_ratio > 0.35:
        return True
    if text_ratio > 0.7:
        return True
    if re.search(r'說明|特性|性能|用途|優勢|注意|描述|Description|Specification|Feature|Note|Formula|Calculation|公式|計算', text, re.I):
        return True
    if len(text) > 40 and text_ratio > 0.4:
        return True
    return False
...
def extract_page_blocks(page_text: str, page_category: str) -> list[dict]:
    ...
    for block in blocks:
        ...
        if is_table_like_block(cleaned) or is_model_block(cleaned):
            continue
        if not (is_formula_block(cleaned) or is_description_block(cleaned)):
            continue
        extracted.append(cleaned)

    # 合併 chunks 到適當長度 (300-1000字元)
    merged = []
    current = ""
